...
def interp_mlp(cfg: DictConfig) -> None:
    ...
    for target_label in range(cfg.data_val.n_class):
        ...
        cp_epoch = 7
        # load checkpoint
        checkpoint = torch.load(os.path.join(cp_dir, f'{cfg.data_val.dataset_name}_mlp_f{fold if cfg.val.n_fold != 'inter' else '_inter'}_wd={cfg.val.mlp.wd}_epoch=7.ckpt'), map_location='cuda:0')
