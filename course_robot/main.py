...
def getinfo(semester_data):
    ...
    course_list = []
    course_types = {'cxjhkxk': "重修和缓考", "fanxk": '计划内选课新生'}
...
if __name__ == '__main__':
    ...
    while route == "" or JSESSIONID == "":
        ...
        userName = "12432852"  # getpass在PyCharm里不能正常工作，请改为input或写死
        passWord = "19260817Zz"
    ...
    semester_info = loads(   # 这里要加mxpylx才能获取到选课所在最新学期
        requests.post('https://tis.sustech.edu.cn/Xsxk/queryXkdqXnxq', data={"mxpylx": 2}, headers=head, verify=False).text)
...
# timing is everything!
    ...
    start = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '12:58:00', '%Y-%m-%d%H:%M:%S')
    end = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '18:35:00', '%Y-%m-%d%H:%M:%S')
    while True:
